...
const onConnectedStripeAccountSuccess = async (accountId: string) => {
     ...
     const stripeAccounts = await User.findOne({
          'stripeAccountInfo.stripeAccountId': accountId,
     });
     ...
     await User.updateOne({ stripeAccountInfo: { stripeAccountId: accountId } });

     const userUpdate = await User.findByIdAndUpdate(
          stripeAccounts._id,
          {
               $set: {
                    'stripeAccountInfo.stripeConnectedAccount': accountId,
     ...
     const html = successHTMLstripeConnection({
          ...
          dashboardLink: `${config.FRONTEND_URL}/seller/overview`,
     });
